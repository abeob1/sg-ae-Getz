...
<%
	If Request.QueryString("DELETE") = "TRUE" Then
		...
		db.Execute "DELETE FROM master_cause WHERE cause_code = '" & Request.QueryString("id") & "'"
		db.Close
		Set db = Nothing
		
		Response.Redirect("swordfish_master_causecode.asp")
	End If
%>
<%
Dim rs_result
...
rs_result.Source = "SELECT cause_code, cause_desc, master_desc  FROM dbo.master_cause, dbo.vw_MasterCauseGroup  WHERE cause_group = master_id  ORDER BY cause_group"
          ...
          <td colspan="4" class="DataGridToolbar"><table width="100%"  border="0" cellspacing="2" cellpadding="1">
            <tr>
              <td><select name="master_switch" class="FormElement" id="master_switch" onChange="document.URL = document.forms[0].master_switch[document.forms[0].master_switch.selectedIndex].value;">
                ...
                <option value="swordfish_master_causecode.asp" selected>Cause Codes</option>
              </select></td>
            </tr>
          </table></td>
        </tr>
...
%>
        <tr>
            <td class="DataGridRow"><%=(rs_result.Fields.Item("cause_code").Value)%></td>
            <td class="DataGridRow"><%=(rs_result.Fields.Item("cause_desc").Value)%></td>
        <td class="DataGridRow"><%=(rs_result.Fields.Item("master_desc").Value)%></td>
            <td align="center" class="DataGridRow"><a href="swordfish_master_causecode.asp?id=<%=(rs_result.Fields.Item("cause_code").Value)%>&DELETE=TRUE" class="DataGridLink" onClick="return confirm('Are you sure?');">Delete</a></td>
        </tr>
        <% 
