...
<%
	If Request.QueryString("DELETE") = "TRUE" Then
		...
		db.Execute "DELETE FROM vw_MasterActivityType WHERE master_id = '" & Request.QueryString("id") & "'"
		db.Close
		Set db = Nothing
		
		Response.Redirect("swordfish_master_cust.asp")
	End If
%>
<%
Dim rs_result
...
rs_result.Source = "SELECT * FROM dbo.vw_MasterCustomer"
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="8">
    ...
    <tr>
      <td>        <table width="100%"  border="0" cellpadding="1" cellspacing="0" class="FormTable">
        <tr>
          <td colspan="6" class="DataGridToolbar"><table width="100%"  border="0" cellspacing="2" cellpadding="1">
            <tr>
              <td><select name="master_switch" class="FormElement" id="master_switch" onChange="document.URL = document.forms[0].master_switch[document.forms[0].master_switch.selectedIndex].value;">
                ...
                <option value="swordfish_master_cust.asp" selected>Customers</option>
        ...
        <td class="DataGridHeader">Name 1 </td>
        <td class="DataGridHeader">Name 2 </td>
        <td class="DataGridHeader">City</td>
        <td class="DataGridHeader">Payment Term </td>
...
%>
        <tr>
            <td class="DataGridRow"><%=(rs_result.Fields.Item("cust_customer").Value)%></td>
            <td class="DataGridRow"><%=(rs_result.Fields.Item("cust_name1").Value)%></td>
        <td class="DataGridRow"><%=(rs_result.Fields.Item("cust_name2").Value)%></td>
            <td class="DataGridRow"><%=(rs_result.Fields.Item("cust_city").Value)%></td>
            <td class="DataGridRow"><%=(rs_result.Fields.Item("TermOfPaymentDesc").Value)%></td>
...
%>
        ...
        <tr>
          <td colspan="6" class="DataGridFooter">&nbsp; Records <%=(rs_result_first)%> to <%=(rs_result_last)%> of <%=(rs_result_total)%>
				<% If MM_offset <> 0 Then %>
