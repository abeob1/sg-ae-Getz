...
<%
	If Request.QueryString("DELETE") = "TRUE" Then
		...
		db.Execute "DELETE FROM master_damage WHERE damage_code = '" & Request.QueryString("id") & "'"
		db.Close
		Set db = Nothing
		
		Response.Redirect("swordfish_master_damagecode.asp")
	End If
%>
<%
Dim rs_result
...
rs_result.Source = "SELECT damage_code, damage_desc, master_desc  FROM dbo.master_damage, dbo.vw_MasterDamageGroup  WHERE master_id = damage_group  ORDER BY damage_group"
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="8">
    ...
    <tr>
      <td>        <table width="100%"  border="0" cellpadding="1" cellspacing="0" class="FormTable">
        <tr>
          <td colspan="4" class="DataGridToolbar"><table width="100%"  border="0" cellspacing="2" cellpadding="1">
            <tr>
              <td><select name="master_switch" class="FormElement" id="master_switch" onChange="document.URL = document.forms[0].master_switch[document.forms[0].master_switch.selectedIndex].value;">
                ...
                <option value="swordfish_master_top.asp">Term of Payments</option>
                ...
                <option value="swordfish_master_damagecode.asp" selected>Damage Codes</option>
...
%>
        <tr>
            <td class="DataGridRow"><%=(rs_result.Fields.Item("damage_code").Value)%></td>
            <td class="DataGridRow"><%=(rs_result.Fields.Item("damage_desc").Value)%></td>
        <td class="DataGridRow"><%=(rs_result.Fields.Item("master_desc").Value)%></td>
            <td align="center" class="DataGridRow"><a href="swordfish_master_damagecode.asp?id=<%=(rs_result.Fields.Item("damage_code").Value)%>&DELETE=TRUE" class="DataGridLink" onClick="return confirm('Are you sure?');">Delete</a></td>
        </tr>
        <% 
...
%>
        ...
        <tr>
          <td colspan="4" class="DataGridFooter">&nbsp; Records <%=(rs_result_first)%> to <%=(rs_result_last)%> of <%=(rs_result_total)%>
				<% If MM_offset <> 0 Then %>
