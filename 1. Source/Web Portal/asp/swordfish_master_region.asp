...
<%
	If Request.QueryString("DELETE") = "TRUE" Then
		...
		db.Execute "DELETE FROM vw_MasterRegion WHERE master_id = '" & Request.QueryString("id") & "'"
		db.Close
		Set db = Nothing
		
		Response.Redirect("swordfish_master_region.asp")
	End If
%>
<%
Dim rs_result
...
rs_result.Source = "SELECT * FROM dbo.vw_MasterRegion"
          ...
          <td colspan="3" class="DataGridToolbar"><table width="100%"  border="0" cellspacing="2" cellpadding="1">
            <tr>
              <td><select name="master_switch" class="FormElement" id="master_switch" onChange="document.URL = document.forms[0].master_switch[document.forms[0].master_switch.selectedIndex].value;">
                ...
                <option value="swordfish_master_region.asp" selected>Region</option>
                <option value="swordfish_master_incoterms.asp">Incoterms</option>
                <option value="swordfish_master_priority">Priority</option>
                <option value="swordfish_master_status">Status</option>
