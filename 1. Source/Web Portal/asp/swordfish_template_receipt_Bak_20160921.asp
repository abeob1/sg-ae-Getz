...
rs_notification.Source = "SELECT dbo.vw_Notification.*, cust_name1, cust_street, cust_city, cust_po, vw_MasterRegion.master_desc AS CustomerRegion, cust_tel1, cust_fax, dbo.vw_MasterStatus.master_desc AS NotificationStatus, user_firstname, user_lastname  FROM dbo.vw_Notification, dbo.vw_MasterCustomer, dbo.vw_MasterStatus, dbo.master_users, vw_MasterRegion WHERE vw_MasterRegion.master_id = cust_region AND cust_customer = notification_soldtoid AND notification_status = dbo.vw_MasterStatus.master_id AND user_id = notification_resp AND vw_Notification.internal_id = '" & Request.QueryString("id") & "'"
rs_notification.CursorType = 1
rs_notification.CursorLocation = 2
rs_notification.LockType = 1
